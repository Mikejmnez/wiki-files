...
When a person finds data somehow and it's available using either an
OPeNDAP server that supports THREDDS catalogs or using http/s from the
Apache web server, those data can potentially be 'brokered' by BALTO
(i.e., they can be served from BALTO as if they were locally stored on
the BALTO site, even though they remain at the remote location.
